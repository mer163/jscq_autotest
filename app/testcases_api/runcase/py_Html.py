...
def passfail(tend):
    ...
    return htl
def ceshixiangqing(id,name,param,url,meth,yuqi,json,relust):
    xiangqing='''
        <tr>
            <td>%s</td>
            <td>%s</td>
    ...
    '''%(id,name,param,url,meth,yuqi,json,passfail(relust))
    return xiangqing
weibu='''
	</table>
...
def relust(titles,starttime,endtime,passge,fail,id,name,param,url,meth,yuqi,json,relust):
    if type(name) ==list:
        relus=' '
        for i in range(len(name)):
            relus+=(ceshixiangqing(id[i],name[i],param[i],url[i],meth[i],yuqi[i],json[i],relust[i]))
        text=title(titles)+connent+time(starttime,endtime,passge,fail)+shanghai+relus+weibu
    else:
        text=title(titles)+connent+time(starttime,endtime,passge,fail)+shanghai+ceshixiangqing(id,name,param,url,meth,int(yuqi),json,relust)+weibu
    return text
def createHtml(filepath,titles,starttime,endtime,passge,fail,id,name,param,url,meth,yuqi,json,relusts):
	texts=relust(titles,starttime,endtime,passge,fail,id,name,param,url,meth,yuqi,json,relusts)
